 ...
 */
static const char *csi_motion_state_text(const csi_processor_result_t *processed)
{
    if (processed == NULL) {
        return "unknown";
    }

    return processed->gain_frozen ? "gain_freeze" :
           (processed->frozen ? "data_frozen" :
           (processed->motion ? "motion" :
            (processed->recovering ? "recover" : "static")));
}
...
{
    printf("%s,%lu,%d,%u,%u,%u,%.2f,%.2f,%.4f,%.4f,%.2f,%.4f,%.4f,%.4f,%u,%u,%d,%.4f,%u,%s\n",
           label,
           ...
           csi_motion_state_text(processed));
}
...
#endif
    printf("CSI_FEATURE_RAW_HEADER,type,frame,rssi,channel,csi_len,valid_points,mean_amp,var_amp,delta_norm,baseline_delta,smooth_mean_amp,motion_score,smooth_motion_score,window_score,freeze_count,agc_gain,fft_gain,compensate_gain,gain_compensated,motion_state\n");
