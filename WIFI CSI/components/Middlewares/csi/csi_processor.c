 ...
 */
typedef enum {
    CSI_STATE_STATIC = 0,  // 静止状态：允许慢速更新 baseline。
    CSI_STATE_MOTION,      // 明显运动状态：禁止更新 baseline，避免把运动学成背景。
    CSI_STATE_RECOVER,     // baseline 自适应恢复状态：运动结束或稳定偏移时快速更新 baseline。
} csi_state_t;
                             ...
                             bool motion_entry_blocked)
{
    ...
    if (state->state == CSI_STATE_STATIC) {
        ...
        if (state->motion_count >= CSI_MOTION_ON_FRAMES) {
            state->state = CSI_STATE_MOTION;
            state->still_count = 0;
            state->last_motion_time_ms = now_ms;
        }
    } else if (state->state == CSI_STATE_MOTION) {
                                             ...
                                             csi_processor_result_t *result)
{
    ...
    if (update_blocked) {
        ...
        csi_update_state(state, activity_score, motion_score, baseline_delta, stable_offset);

        if (stable_offset_ready) {
            if (state->state != CSI_STATE_RECOVER) {
                state->state = CSI_STATE_RECOVER;
                state->motion_count = 0;
                state->still_count = 0;
        ...
        if (state->state == CSI_STATE_RECOVER && activity_score < CSI_RECOVER_ACTIVITY_THRESHOLD) {
            ...
            csi_update_baseline(state->baseline_norm_values,
                                ...
                                valid_points,
                                alpha);
        } else if (state->state == CSI_STATE_STATIC &&
    ...
    bool moving_now = !gain_freeze && state->state == CSI_STATE_MOTION;
    bool recovering = !gain_freeze && state->state == CSI_STATE_RECOVER;
