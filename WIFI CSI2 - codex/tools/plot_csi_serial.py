...
class CsiFeature:
    ...
    band3_score: float = 0.0
    band0_state_id: int = 0
    band1_state_id: int = 0
    band2_state_id: int = 0
    band3_state_id: int = 0
...
def parse_csi_feature(line: str) -> CsiFeature | None:
    ...
    try:
        ...
        return CsiFeature(
            ...
            band3_score=band_scores[3],
            band0_state_id=band_states[0],
            band1_state_id=band_states[1],
            band2_state_id=band_states[2],
            band3_state_id=band_states[3],
        )
    except ValueError:
        return None
...
def append_csv(csv_writer: csv.writer, feature: CsiFeature) -> None:
    ...
    csv_writer.writerow([
        ...
        feature.global_state_id,
        feature.subband_state_id,
        feature.fusion_state_id,
        feature.band0_state_id,
        feature.band1_state_id,
        feature.band2_state_id,
        feature.band3_state_id,
        feature.decision_score,
...
def run_plot(args: argparse.Namespace) -> None:
    ...
    band0_score = deque(maxlen=args.window)
    band1_score = deque(maxlen=args.window)
    band2_score = deque(maxlen=args.window)
    band3_score = deque(maxlen=args.window)
    ...
    (line_band0,) = ax_motion.plot([], [], label="band0_score", linewidth=1.0, color=colors["band0"], alpha=0.75)
    (line_band1,) = ax_motion.plot([], [], label="band1_score", linewidth=1.0, color=colors["band1"], alpha=0.75)
    (line_band2,) = ax_motion.plot([], [], label="band2_score", linewidth=1.0, color=colors["band2"], alpha=0.75)
    (line_band3,) = ax_motion.plot([], [], label="band3_score", linewidth=1.0, color=colors["band3"], alpha=0.75)
    ...
    for line in (line_delta, line_base, line_score, line_smooth, line_window, line_band0, line_band1, line_band2, line_band3, line_rssi):
    ...
    def update(_: int):
        ...
        while True:
            ...
            band0_score.append(latest.band0_score)
            band1_score.append(latest.band1_score)
            band2_score.append(latest.band2_score)
            band3_score.append(latest.band3_score)
            last_feature = latest
        ...
        line_band0.set_data(x, list(band0_score))
        line_band1.set_data(x, list(band1_score))
        line_band2.set_data(x, list(band2_score))
        line_band3.set_data(x, list(band3_score))
